export default function About() {
  return (
    <section
      id="about"
      className="max-w-3xl mx-auto py-16 px-6 text-center md:text-left"
    >
      <h2 className="text-3xl font-bold mb-6">About Me</h2>
      <p className="text-gray-700 leading-relaxed mb-4">
        A software engineer with experience across safety-critical systems
        and modern web applications. I enjoy turning complex problems into
        clean, maintainable solutions.
      </p>
      <p className="text-gray-700 leading-relaxed">
        My expertise spans React, TypeScript, Next.js, and Python. I have built
        projects like <strong>Nurture Patch</strong> and <strong>Gustat</strong>,
        delivering practical and high-quality solutions. I thrive in agile teams
        and enjoy mentoring and collaborating to create impactful products.
      </p>
    </section>
  );
}
